...
{
    // Colors are 0=black, 1=blue, 2=green, and so on up to 15=white
    // colorattribute = foreground + background * 16
    // Example: To get red text on yellow, use 4 + 14*16 = 228
...
bool test1Shift()
{
    ...
    try
    {
        ...
        if (got != expected)
        {
            set_console_color(RED);
            cout << "FAILED: ShiftText information is not as expected\n" <<
                "Not supposed to encrypt twice\n";
            set_console_color(WHITE);
            return false;
        }

        cout << "\nInitializing ShiftText object s2 with text = \"lmnopqrstuvwxyz\" , key = 5 ... \n" << endl;
        ...
        if (got != expected)
        {
            ...
            cout << "FAILED: CaesarText information is not as expected\n" <<
                "Check functions CaesarText(text), PlainText::getText(), PlainText::isEncrypted()\n";
            set_console_color(WHITE);
            return false;
        }
    ...
    catch (...)
    {
        ...
        std::cout << "FAILED: The program crashed. Check the following things:\n" <<
            ...
            "3. Did you remember to initialize arrays before accessing them?";
        set_console_color(WHITE);
        return false;
    }
